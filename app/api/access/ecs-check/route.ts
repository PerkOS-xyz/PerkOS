/**
 * GET /api/access/ecs-check
 *
 * Tells the agent wizard whether the caller is allowed to pick the
 * "PerkOS infra (AWS ECS)" deploy mode.
 *
 * Sources of truth, in order:
 *   1. Super-admins (env `PERKOS_SUPER_ADMINS` + Firestore /super_admins)
 *      — always allowed.
 *   2. Firestore /ecs_allowlist/{address} — managed via PerkOS-Admin
 *      /ecs-access. Manual allow list until billing (x402 monthly) lands.
 *
...
export async function GET(request: Request) {
  ...
  try {
    const snap = await adminDb().collection("ecs_allowlist").doc(caller).get();
    if (snap.exists) {
