...
Remote LiveKit audio attaches to an explicit hidden browser audio element configured for autoplay and inline, unmuted playback. The controller calls `play()`, surfaces only safe connected/playing/permission status, and detaches, pauses, and removes the element on unsubscribe, end, failure, or unmount.
